// Modified version of ActiveRunPage without pace information
import 'dart:async';
import 'dart:convert';
import 'dart:math';
...
class ActiveRunPageState extends State<ActiveRunPage> {
  ...
  static const _resumeThreshold = 1.0;  // m/s
  ...
  Future<void> _initializeRun() async {
    ...
    _runTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (!_autoPaused && !_manuallyPaused && mounted) {
        setState(() {
          _secondsElapsed++;
          // Update calories every 10 seconds (simple estimation)
  ...
  void _handleAutoPauseLogic(double speed) {
    ...
    if (_autoPaused) {
      ...
      if (speed < _pauseThreshold) {
        _stillCount++;
        if (_stillCount >= 5) {  // 5 consecutive low-speed readings
          setState(() {
            _autoPaused = true;
          });
